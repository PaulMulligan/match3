...
window.onload = function() {
        ...
        for (var i=0; i<level.columns; i++) {
            for (var j=0; j<level.rows; j++) {
                ...
                if (level.tiles[i][j].type >= 0) {
                    ...
                    if (level.tiles[i][j].isNew && gamestate == gamestates.resolve) {
                        // Draw the tile popping into existence with the color
                        drawPartialTile(coord.tilex, coord.tiley, col[0], col[1], col[2], i, j);
                    } else {
    ...
    function drawPartialTile(x, y, r, g, b, i, j) {
        if (animationtime >= animationtimetotal*0.75) {
            level.tiles[i][j].isNew = false;
        }
        var ratio = animationtime / animationtimetotal;
        var halfwidth = level.tilewidth / 2;
        var halfheight = level.tileheight / 2;
        context.fillStyle = "rgb(" + r + "," + g + "," + b + ")";
